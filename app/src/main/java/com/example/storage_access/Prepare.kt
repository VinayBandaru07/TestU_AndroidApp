...
import com.example.storage_access.databinding.ActivityPrepareBinding
...
//        var lines: List<String> = File(Target_file_path).readLines()
        ...
        var totalQuestions: Int = questionList.size
        ...
        binding.questionView.setOnClickListener() {
            makeColor(answerList[presentIndex].toString())

        }


        fun makeThePaper() {
            binding.questionView.text = questionList[presentIndex]
            binding.option1View.text = option_1[presentIndex]
            binding.option2View.text = option_2[presentIndex]
            binding.option3View.text = option_3[presentIndex]
            binding.option4View.text = option_4[presentIndex]

        }
        makeThePaper()

        fun makeColorsNormal() {
            binding.PresentQuestion.text = "Question-" + (presentIndex - 1).toString()
        ...
        binding.nextPage.setOnClickListener() {
            if ((questionList.size - 1) > presentIndex) {
                ...
                if (binding.GOTO.text.toString() == "") {
                    ...
                    if (binding.GOTO.text.toString().toInt() <= totalQuestions) {
                        ...
                        presentIndex = binding.GOTO.text.toString().toInt()+1
                        makeColorsNormal()
                        binding.GOTO.text = null
                ...
                builder.setTitle("Preparation Completed")
                //set message for alert dialog
                builder.setMessage("Wanna Take a Test ?")
                builder.setPositiveButton("Okay") { dialogInterface, which ->
                    val i = Intent(this, quizLogo::class.java)
                    i.putExtra("SelectedPosition", FilePosition)
                    startActivity(i)
                }
        ...
        val builder = AlertDialog.Builder(this)
        builder.setMessage("1.Idhi Preparation Mode\n" +
                "2.Indulo 200 bits untay\n" +
                "3.You can get the answer by clicking on the Question\n" +
                "4.This is the trail version and development of app is almost done\n" +
                "5.Emaina bugs unte give me your most valuable Feedback😊"
                )
        val alertDialog: AlertDialog = builder.create()
        alertDialog.show()
